...
function loadRegion(value){
    ...
    for (var i = 0; i < value.length; i++) {
        valueHtml += ' <li class="filter">'+
                            '<label>';
        valueHtml += '<input type="checkbox" class="myf" name="region" data-value="'+value[i].region+'" value=".grid-item--'+value[i].region+'"/>'+
                            value[i].region
        valueHtml += '   </label></li>';
    }
...
function loadRobeVin(value){
    ...
    for (var i = 0; i < value.length; i++) {
        valueHtml += ' <li class="filter">'+
        '<label>';
        valueHtml += '<input type="checkbox" class="myf" name="color" data-value="'+value[i].color+'" value=".grid-item--'+value[i].color+'"/>'+
                            value[i].color
        valueHtml += '   </label></li>';
    }
...
function loadCategorie(value){
    ...
    for (var i = 0; i < value.length; i++) {
        valueHtml += ' <li class="filter">'+
                            '<label>';
        valueHtml += '<input type="checkbox" class="myf" name="categorie" data-value="'+value[i].price_range+'" value=".grid-item--'+value[i].price_range+'"/>'+
                            value[i].price_range;
        valueHtml += '   </label></li>'
    }
...
var countChecked = function() {
  ...
  if (parseInt(n) > 0){
    ...
    console.log('attr'+  $( "input[name*="+$(this).attr('name')+"]" ).prop( "checked" ))
    console.log()
    data.push(selected)
